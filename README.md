...
- **多引擎文档流水线** — Docling、MinerU、Insavlo 三套解析引擎按文档类型自动路由，覆盖 Office、PDF、图片等全格式。流水线全程可视化，每个环节的状态与耗时均可回溯。
- **RAPTOR 分层检索** — 引入斯坦福 RAPTOR 算法对已索引文档做递归抽象与分层摘要，构建树形语义结构，大幅提升长文档与多文档场景下的检索精度。
